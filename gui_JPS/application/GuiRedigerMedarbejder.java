...
public class GuiRedigerMedarbejder implements Initializable {
	...
	public void initialize(URL fxmlFileLocation, ResourceBundle resources) {
		...
		try {
			navn.setText(sysoOp.findMedarbejderID(ID).get(0).toString());
			email.setText(sysoOp.findMedarbejderID(ID).get(1).toString());

			ObservableList oListItems = FXCollections.observableArrayList(sysoOp.redigeringsKompetencer(ID));
			items = oListItems;
			LW.setItems(items);

			for (Object b : items) {
				kompetencerAdder((String) b);
			}
		...
		btn2.setOnAction(ae -> {
			try {
				re.redigerMedarbejder(navn.getText(), email.getText(), kompetencerValgt);
